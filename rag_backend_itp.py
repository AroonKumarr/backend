...
try:
    ...
    logging.error(f"Embedder initialization failed: {e}")
    # embedder stays None → endpoints will handle this
...
try:
    ...
    logging.error(f"Qdrant client initialization failed: {e}")
    # client stays None → endpoints will handle this
...
async def stt_endpoint(file: UploadFile = File(...)):
    ...
    safe_name = os.path.basename(file.filename)  # sanitize filename
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("rag_backend_itp:app", host="0.0.0.0", port=PORT)
